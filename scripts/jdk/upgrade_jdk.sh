...
DB_JDK_COPY_DEST_DIR=""
DB_UPDATE_SCRIPT_SOURCE=""
DB_UPDATE_CONFIG_SOURCE=""
...
load_config() {
  ...
  ensure_array_defined DB_JDK_COPY_HOSTS
...
build_remote_db_update_command() {
  local command="${DB_UPDATE_COMMAND}"
  local arg=""
  local args=(
    --env "${ENV_NAME}"
    --config "${DB_UPDATE_REMOTE_CONFIG}"
  ...
  for arg in "${args[@]}"; do
    command+=" $(format_display_arg "${arg}")"
  done
  printf '%s\n' "${command}"
}
...
step_8_update_database_hosts() {
  ...
  if [[ "${#DB_JDK_COPY_HOSTS[@]}" -eq 0 ]]; then
    log_skip "No DB JDK copy hosts configured for ${ENV_NAME}"
  else
    for host in "${DB_JDK_COPY_HOSTS[@]}"; do
      run_logged_cmd scp -qr "${DB_JDK_COPY_SOURCE_PATH}" "${host}:${DB_JDK_COPY_DEST_DIR}"
    done
  fi
  ...
  remote_command="$(build_remote_db_update_command)"
  for host in "${DB_UPDATE_HOSTS[@]}"; do
    run_logged_cmd scp -q "${DB_UPDATE_SCRIPT_SOURCE}" "${host}:${DB_UPDATE_REMOTE_SCRIPT}"
    run_logged_cmd scp -q "${DB_UPDATE_CONFIG_SOURCE}" "${host}:${DB_UPDATE_REMOTE_CONFIG}"
    run_logged_cmd ssh -q "${host}" -C "chmod +x ${DB_UPDATE_REMOTE_SCRIPT}"
    run_logged_cmd ssh -q "${host}" -C "${remote_command}"
  done
}
